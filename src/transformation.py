...
# BUSINESS METRICS
# -------------------------
def calculate_revenue_per_user(transactions: pd.DataFrame) -> pd.DataFrame:
    df = transactions.copy()

    # considerar apenas transações com sucesso
    ...
    revenue_per_user.rename(columns={"amount": "revenue"}, inplace=True)

    logging.info("Revenue per user calculated")
...
def run_transformation(users, transactions, events):
    ...
    # NOVO
    revenue_per_user = calculate_revenue_per_user(transactions)
